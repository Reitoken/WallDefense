...
- 19 automated tests passing (`WallDefense.Health.*`, `WallDefense.Hero.*`, `WallDefense.Weapons.*`).
- **Next: Step 4** — monsters: `AWDMonster` + move patterns + `DA_Monster` (zone 1 bestiary) + `UWDStageDirector` + `DA_Stage` waves + hit response (`DA_HitResponse`, `GameFeelSubsystem`, damage numbers).
